...
class FeelingButton {
    private String text;
    private int id;
    private int pos_neg_id;

    public FeelingButton(String text, int id, int pos_neg_id){
        this.text = text;
        this.id = id;
        this.pos_neg_id = pos_neg_id;
    }
...
class FeelingButtonAdapter extends RecyclerView.Adapter<ButtonViewHolder> {
    ...
    public void onBindViewHolder(@NonNull ButtonViewHolder holder, int position) {
        ...
        holder.dbutton.setGravity(Gravity.CENTER);
        ...
        viewModel.getDuasById(feelingButton.getId()).observe(lifecycleOwner, new Observer<List<Dua>>() {
            ...
            public void onChanged(List<Dua> duas) {
                if (duas != null) {
                    ...
                    fragmentTransaction.add(R.id.frameLayout, duaCardFragment);
    ...
    public void setButtons(List<FeelingButton> feelingButtons){
        this.feelingButtons = feelingButtons;
    }
...
class DuaAdapter extends RecyclerView.Adapter<DuaViewHolder> {
    ...
    public void onBindViewHolder(@NonNull DuaViewHolder holder, int position) {
        ...
        if (dua.favorite == 1) {
            holder.addToFav.setBackground(ResourcesCompat.getDrawable(holder.itemView.getResources(), R.drawable.ic_baseline_favorite_24, null));
        } else {
            holder.addToFav.setBackground(ResourcesCompat.getDrawable(holder.itemView.getResources(), R.drawable.heart_svgrepo_com, null));
        }
...
//
//        // Set text and handle favorite button
//        if (dua != null) {
//            holder.arabicText.setText(dua.arabic);
//            holder.englishText.setText(dua.english);
//
...
//            }
//
//            handleFavoriteButton(holder, dua);
//        }
//    }
    }
...
//
//    private void handleFavoriteButton(DuaViewHolder holder, Dua dua) {
//        if (dua.favorite == 0) {
...
//        }
//
//        holder.addToFav.setOnClickListener(v -> toggleFavorite(dua, holder));
//    }
//
//    private void toggleFavorite(Dua dua, DuaViewHolder holder) {
//        if (dua.favorite == 0) {
//            viewModel.addFavoriteAndUpdateDua(dua.id);
//            holder.addToFav.setBackground(ResourcesCompat.getDrawable(holder.itemView.getResources(), R.drawable.heart_svgrepo_com, null));
//            Toast.makeText(holder.itemView.getContext(), "Dua added to favorites", Toast.LENGTH_SHORT).show();
